...
export type TAsyncActionCreator<T extends string, P> = (
  payload: P
) => IAction<P, T>;
export type TRejectedActionCreator<T extends string, P> = (
  payload: P,
  message: string
) => IAction<P, T>;
...
>(
  ...
  function builder<P1, P2, P3, P4>(): IAsyncActionsBuilder<
    T1,
    T2,
    T3,
    T4,
    ...
    P4
  > {
    const invokeActionCreator = (payload: P1): IAction<P1, T1> => ({
      type: baseType,
      payload
    });
    invokeActionCreator.getType = () => baseType;

    const fulfilledActionCreator = (payload: P3): IAction<P3, T3> => ({
      type: fulfilledType,
      payload
    });
    fulfilledActionCreator.getType = () => fulfilledType;

    const pendingActionCreator = (payload: P2): IAction<P2, T2> => ({
      type: pendingType,
      payload
    });
    pendingActionCreator.getType = () => pendingType;

    const rejectedActionCreator = (
      payload: P4,
      message: string
    ): IAction<P4, T4> => ({ type: rejectedType, message, payload });
